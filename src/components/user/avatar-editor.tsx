...
type ProfileResponse = { settings?: UserSettings };
...
export function AvatarEditor({
  ...
  async function uploadFile(file: File): Promise<string> {
    ...
    form.set("file", file);
    const data = await settingsRequest<{ mediaKey?: unknown }>(
      "/api/media",
      { method: "POST", body: form },
      "Upload failed"
    );
    if (typeof data.mediaKey !== "string" || !data.mediaKey) {
  ...
  async function persistImage(next: string | null) {
    const data = await settingsRequest<ProfileResponse>(
      "/api/me/profile",
      {
        method: "PATCH",
    ...
    if (!savedSettings) throw new Error("Could not save avatar");
    setPreview(savedSettings.image);
    onSaved?.(savedSettings.image);
  ...
  function runImageUpdate(next: string | null) {
    ...
    startTransition(async () => {
      try {
        ...
        setError(
          settingsErrorMessage(cause, localizeError, t("common.error"))
        );
      }
    });
  }
