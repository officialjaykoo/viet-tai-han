...
## 23. Bug12 current gate notes

The current release gate runs the following commands from a clean local D1 state:
...
`scripts/audit-db-integrity.mjs` is read-only. It checks foreign-key violations, all five canonical counter drifts, relation orphans, legacy row counts, and—when `--schema` is supplied—table definitions, columns, foreign keys, and explicit indexes. Use `node scripts/audit-db-integrity.mjs --remote --strict` after deployment. Migration `0043_remove_legacy_feed_indexes.sql` is forward-only and removes only proven dead pre-canonical indexes; legacy vote and score/hot-score data remain retained when production evidence finds rows.

The browser fixture uses separate Chromium contexts for allowlisted seeded `alice` and `bob` accounts. The E2E session endpoint is enabled only by the explicit test bypass and rejects other usernames. Production does not enable that bypass. Local runs skip the DM WebSocket journey when `PLAYWRIGHT_BASE_URL` is absent because the `CHAT_ROOM` Durable Object binding is unavailable in Next dev; a deployed test environment must prove request/accept/reply through the rendered WebSocket path.

The critical repeat gate is:
...
New tests must use locator/URL/response assertions or `expect.poll`, not arbitrary sleeps. The existing named `warmBotGuard` dwell is the only intentional fixed wait.
