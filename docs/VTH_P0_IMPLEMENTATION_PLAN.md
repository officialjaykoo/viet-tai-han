...
- local DB: migrations `0001`~`0035`, `seed.sql`
...
| 2 | `wrangler.jsonc` | Worker를 `vth`로 변경. D1/R2 리소스와 Workers AI(번역 전용) binding을 사용하고 zero D1 ID는 실제 계정 생성 전까지 placeholder로 남긴다. 운영 deploy는 placeholder를 거부한다. |
...
| `src/lib/messages.ts`, `src/lib/actions.ts`, `src/lib/qna.ts`, `src/lib/marketplace.ts`, `src/lib/businesses.ts` | pair creation unique conflict를 409로 변환하고, posts/comments/listings/questions/answers/business/booking/DM writes는 `Idempotency-Key` 또는 body request ID를 `0035_write_idempotency`에 저장해 retry 시 같은 row를 반환한다. request response는 조건부 상태 전이와 D1 재조회를 거쳐 중복 notification/broadcast를 막는다. |
| `src/worker.ts`, `wrangler.jsonc`, package preview/deploy | custom worker가 OpenNext generated handler를 감싸고 ChatRoom을 직접 import/export한다. active PostObject binding/export는 제거했으며, historical DO migration metadata만 유지한다. `wrangler deploy --dry-run` bundle에서 ChatRoom export를 확인한다. |
...
6. Vietnamese content translation and D1 discovery — **완료**. `0025_multilingual_content.sql`의 translation metadata, vi/ko/en/ru 감지·Workers AI 번역, D1 기반 최신순/활동 기반 추천 fallback을 적용했다.
...
- 게시물 카드는 Facebook-style의 작성자 헤더, 본문 중심 흰색 카드, 가로형 반응·댓글 action row를 사용한다. 좋아요 API와 reputation 데이터는 분리한다.
...
- 사용자 노출 Reddit 시각 잔재는 제거하되, `/r/*`, `/u/*`, `subreddits` 내부 호환 계약은 유지한다.
- 광고·Pro·결제 등 직접적인 commercial surface는 제품 규모가 커질 때까지 보류하고 기본 비활성 상태를 유지한다.

## 외부 blocker

- Cloudflare account ID와 실제 D1/R2/Workers AI/KV/rate-limit resource.
